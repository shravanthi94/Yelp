...
const OrdersCancelled = ({ order: { allOrders }, setAlert }) => {
  ...
  let cancelledOrders = allOrders.filter(
    (order) => order.order_type === 'CANCELLED',
  );
    ...
    <Fragment>
      <div className='container'>
        <h1 className={styles.order_title}>
          CANCELLED Orders placed by customers
        </h1>
...
export default connect(mapStateToProps, { setAlert })(OrdersCancelled);
