...
const OrdersPick = ({ orders: { allorders, loading }, setAlert }) => {
  const displayOrders = (orders) => {
    orders = orders.filter(
      (order) =>
        order.order_status === 'PICKUP' || order.order_status === 'PICKED UP',
    );
    if (orders.length === 0) {
...
export default connect(mapStateToProps, { setAlert })(OrdersPick);
